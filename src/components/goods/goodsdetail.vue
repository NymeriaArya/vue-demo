...
<script>
	...
	import prodTools from '../common/prod.js';
    export default {
        data(){
            return {
        ...
        created(){
            ...
            this.$axios.get('getgoodsimages?id='+id)
			.then(res=>{
				this.imgs=res.data;
			},err=>{
				console.log(err);
			...
			this.goodsDetailUrl='getgoodsimages?id='+id;
			
			//获取详情数据
			this.$axios.get('getinfo?id='+id)
			.then(res=>{
				this.prodInfo=res.data[0];
			},err=>{
				console.log(err);
			})
			},
